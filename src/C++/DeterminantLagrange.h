...
lli lagrangeCofDeterminantCounting(Matriz mtz, int n, lli *numDetCalculados, int* numDetOrden) {
    lli det = 0;
    if (n == 2) {
        det = mtz[0][0] * mtz[1][1] - mtz[0][1] * mtz[1][0];
        (*numDetCalculados)++;
        ...
        for (int i = 0; i < n; i++) {
            ...
            //    continue;
            Matriz aux = getComplementaryMatrixLagrange(mtz, n, i);
            det += (i%2==0 ? 1 : -1) * pivot * lagrangeCofDeterminantCounting(aux, n - 1, numDetCalculados, numDetOrden);
        }
        numDetOrden[n-2]++;
    }
    return det;
